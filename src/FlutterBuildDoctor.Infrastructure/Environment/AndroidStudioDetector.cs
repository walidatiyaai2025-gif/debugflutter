...
public sealed class AndroidStudioDetector : IAndroidStudioDetector
{
    ...
    public AndroidStudioDetectionResult Detect(WindowsEnvironmentInfo windowsEnvironment)
    {
        ...
        {
            return new AndroidStudioDetectionResult(
                AndroidStudioDetectionStatus.NotWindows,
                Array.Empty<AndroidStudioInstallation>(),
                "Android Studio Windows installation discovery is unavailable because the current operating system is not Windows.");
        }

        if (!windowsEnvironment.IsSuccess)
        {
            return new AndroidStudioDetectionResult(
                AndroidStudioDetectionStatus.InspectionFailed,
                Array.Empty<AndroidStudioInstallation>(),
                "Android Studio discovery requires successful Windows environment detection.");
        }

        IReadOnlyList<AndroidStudioExecutableEvidence> discovered;
        try
        {
            discovered = _source.Discover();
        }
        catch (Exception ex)
        {
            return new AndroidStudioDetectionResult(
                AndroidStudioDetectionStatus.InspectionFailed,
                Array.Empty<AndroidStudioInstallation>(),
                $"Android Studio installation locations could not be inspected: {ex.Message}");
        }
        ...
        if (discovered.Count == 0)
        {
            return new AndroidStudioDetectionResult(
                ...
                Array.Empty<AndroidStudioInstallation>(),
                "No Android Studio installation was found in the supported Windows installation roots.");
        }

        var installations = discovered
            .Select(BuildInstallation)
            .OrderBy(installation => installation.DiscoverySource)
            .ThenByDescending(installation => ParseVersion(installation.Version))
            .ThenBy(installation => installation.InstallationPath, StringComparer.OrdinalIgnoreCase)
            .ToArray();

        var incomplete = installations.Count(installation => string.IsNullOrWhiteSpace(installation.Version));
        var suffix = incomplete > 0
            ? $" {incomplete} installation(s) have incomplete version metadata but remain visible as evidence."
            : string.Empty;
        return new AndroidStudioDetectionResult(
            AndroidStudioDetectionStatus.Succeeded,
            installations,
            $"Detected {installations.Length} Android Studio installation(s).{suffix}");
    }
    ...
    private static AndroidStudioInstallation BuildInstallation(AndroidStudioExecutableEvidence evidence)
    {
        ...
        var installationPath = !string.IsNullOrWhiteSpace(bin) &&
                               string.Equals(Path.GetFileName(bin), "bin", StringComparison.OrdinalIgnoreCase)
            ? Directory.GetParent(bin)?.FullName ?? bin
            : bin ?? executablePath;
        ...
        if (productInfoPath is not null)
        {
            ...
            {
                return new AndroidStudioInstallation(
                    executablePath,
                    installationPath,
            ...
            {
                RawMetadata = raw,
                Message = JoinMessages(error, fallback.Message)
            };
        }
    ...
    private static AndroidStudioInstallation BuildFromFallback(
        ...
        AndroidStudioDiscoverySource discoverySource)
    {
        ...
        if (File.Exists(buildPath))
        {
            try
            {
                ...
                return new AndroidStudioInstallation(
                    executablePath,
                    installationPath,
                    file.ProductName ?? "Android Studio",
                    file.Version,
                    buildNumber.Length == 0 ? null : buildNumber,
                    ProductCodeFromBuild(buildNumber),
                    discoverySource,
                    AndroidStudioMetadataSource.BuildTxt,
                    raw,
                    buildNumber.Length == 0 ? "build.txt is empty." : file.Message);
            }
            catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
            {
                ...
                return new AndroidStudioInstallation(
                    ...
                    null,
                    discoverySource,
                    file.Version is null ? AndroidStudioMetadataSource.None : AndroidStudioMetadataSource.ExecutableFileVersion,
                    null,
        ...
        return new AndroidStudioInstallation(
            executablePath,
            installationPath,
            fileVersion.ProductName ?? "Android Studio",
            fileVersion.Version,
            null,
            null,
            discoverySource,
            fileVersion.Version is null ? AndroidStudioMetadataSource.None : AndroidStudioMetadataSource.ExecutableFileVersion,
            null,
    ...
    private static string? FindProductInfo(string installationPath, string? binPath)
    {
        foreach (var candidate in new[]
                 {
                     Path.Combine(installationPath, "product-info.json"),
                     string.IsNullOrWhiteSpace(binPath) ? null : Path.Combine(binPath, "product-info.json")
                 })
        {
    ...
    private static ProductInfo? TryReadProductInfo(string path, out string? raw, out string? error)
    {
        ...
        try
        {
            ...
            {
                error = "product-info.json did not contain recognizable Android Studio identity fields.";
                return null;
            }
        ...
        {
            error = $"product-info.json could not be parsed: {ex.Message}";
            return null;
        }
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
        {
            error = $"product-info.json could not be read: {ex.Message}";
            return null;
        }
    }
    ...
    private static FileVersionEvidence TryReadFileVersion(string executablePath)
    {
        try
        {
            ...
            var version = FirstNonBlank(info.ProductVersion, info.FileVersion);
            return new FileVersionEvidence(
                FirstNonBlank(info.ProductName, info.FileDescription),
                version,
                null);
        }
    ...
    private sealed record FileVersionEvidence(
        string? ProductName,
        string? Version,
        string? Message);
}
