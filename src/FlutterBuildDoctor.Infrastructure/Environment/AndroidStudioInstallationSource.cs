...
public sealed record AndroidStudioSearchRoot(
    string Path,
    AndroidStudioDiscoverySource Source,
    bool Recursive,
    int MaxDepth = 5);
...
public sealed class SystemAndroidStudioSearchRootProvider : IAndroidStudioSearchRootProvider
{
    public IReadOnlyList<AndroidStudioSearchRoot> GetRoots()
    {
        ...
        {
            roots.Add(new AndroidStudioSearchRoot(
                Path.Combine(localAppData, "Programs", "Android Studio"),
                AndroidStudioDiscoverySource.LocalAppDataPrograms,
                Recursive: false));
            roots.Add(new AndroidStudioSearchRoot(
                Path.Combine(localAppData, "JetBrains", "Toolbox", "apps", "AndroidStudio"),
                AndroidStudioDiscoverySource.JetBrainsToolbox,
                Recursive: true,
                MaxDepth: 6));
        }
    ...
    private static void AddDirectRoot(
        ICollection<AndroidStudioSearchRoot> roots,
        string basePath,
        AndroidStudioDiscoverySource source)
    {
        ...
        roots.Add(new AndroidStudioSearchRoot(
            Path.Combine(basePath, "Android", "Android Studio"),
            source,
            Recursive: false));
    }
}
...
public sealed class WindowsAndroidStudioInstallationSource : IAndroidStudioInstallationSource
{
    ...
    public IReadOnlyList<AndroidStudioExecutableEvidence> Discover()
    {
        ...
        foreach (var root in _rootProvider.GetRoots())
        {
            ...
            if (root.Recursive)
                DiscoverRecursive(root, results);
            else
                AddExecutableFromInstallationRoot(root.Path, root.Source, results);
        }

        return results.Values
            .OrderBy(evidence => evidence.DiscoverySource)
            .ThenBy(evidence => evidence.ExecutablePath, PathComparer)
            .ToArray();
    }

    private static void DiscoverRecursive(
        AndroidStudioSearchRoot root,
        IDictionary<string, AndroidStudioExecutableEvidence> results)
    {
        ...
        while (pending.Count > 0)
        {
            ...
            if (current.Depth >= root.MaxDepth)
                continue;
    ...
    private static void AddExecutableFromInstallationRoot(
        string installationRoot,
        AndroidStudioDiscoverySource source,
        IDictionary<string, AndroidStudioExecutableEvidence> results)
    {
        var bin = string.Equals(Path.GetFileName(installationRoot), "bin", StringComparison.OrdinalIgnoreCase)
            ? installationRoot
            : Path.Combine(installationRoot, "bin");
