...
public sealed class PubspecLockParser : IPubspecLockParser
{
    ...
    public PubspecLockParseResult Parse(FlutterProjectRootResult projectRoot)
    {
        ...
        {
            return Result(PubspecLockParseStatus.ProjectRootUnavailable, projectRoot, null, null,
        ...
        {
            return Result(PubspecLockParseStatus.InvalidRequest, projectRoot, null, null,
                $"Resolved project path is invalid: {ex.Message}");
        }

        if (!File.Exists(lockPath))
        {
            return Result(PubspecLockParseStatus.LockFileNotFound, projectRoot, lockPath, null,
        ...
        try
        {
            ...
            {
                return Result(PubspecLockParseStatus.InvalidRequest, projectRoot, lockPath, null,
            ...
            {
                return Result(PubspecLockParseStatus.FileTooLarge, projectRoot, lockPath, null,
        ...
        {
            return Result(PubspecLockParseStatus.ReadFailed, projectRoot, lockPath, null,
        ...
        {
            return Result(PubspecLockParseStatus.MalformedYaml, projectRoot, lockPath, rawText,
        ...
        {
            return Result(PubspecLockParseStatus.InvalidDocument, projectRoot, lockPath, rawText,
        ...
        if (ValidateSdkShape(root) is { } sdkShapeError)
            return Result(PubspecLockParseStatus.InvalidDocument, projectRoot, lockPath, rawText, sdkShapeError);
        ...
        foreach (var pair in packagesNode.Children)
        {
            ...
            if (ValidatePackageShape(packageName, packageNode) is { } packageShapeError)
                return Result(PubspecLockParseStatus.InvalidDocument, projectRoot, lockPath, rawText, packageShapeError);
            ...
            if (string.IsNullOrWhiteSpace(version) || string.IsNullOrWhiteSpace(sourceText) || string.IsNullOrWhiteSpace(dependencyType))
            {
                return Result(PubspecLockParseStatus.InvalidDocument, projectRoot, lockPath, rawText,
            ...
            {
                if (source == PubspecLockedPackageSource.Path) descriptionPath = scalarDescription.Value;
                else if (source == PubspecLockedPackageSource.Sdk) descriptionName = scalarDescription.Value;
            }
            else if (description is YamlMappingNode descriptionMap)
            {
                ...
                gitRef = source == PubspecLockedPackageSource.Git ? Scalar(descriptionMap, "ref") : null;
                gitResolvedRef = source == PubspecLockedPackageSource.Git ? Scalar(descriptionMap, "resolved-ref") : null;
                gitUrl = source == PubspecLockedPackageSource.Git ? SanitizeUrlEvidence(Scalar(descriptionMap, "url")) : null;
            }

            packages.Add(new PubspecLockedPackage(packageName, version, source, dependencyType,
                descriptionName, descriptionUrl, descriptionPath, sha256, gitRef, gitResolvedRef, gitUrl));
        }
        ...
        return new PubspecLockParseResult(PubspecLockParseStatus.Succeeded, projectRoot, lockPath, metadata, rawText,
    ...
    {
        if (Node(root, "sdks") is not { } sdksNode) return null;
        if (sdksNode is not YamlMappingNode sdks) return "pubspec.lock section 'sdks' must be a mapping.";
    ...
    private static PubspecLockedPackageSource ParseSource(string value) => value.ToLowerInvariant() switch
    {
    ...
    private static string? Scalar(YamlMappingNode mapping, string key) => (Node(mapping, key) as YamlScalarNode)?.Value;
    ...
    private static string? SanitizeUrlEvidence(string? value)
    {
        ...
        {
            if (string.IsNullOrEmpty(uri.UserInfo) && string.IsNullOrEmpty(uri.Query) && string.IsNullOrEmpty(uri.Fragment)) return trimmed;
            try
            {
            ...
            catch (UriFormatException) { return "[redacted-url]"; }
        }
        var atIndex = trimmed.IndexOf('@');
        if (atIndex > 0 && trimmed[..atIndex].Contains(':')) return "[redacted]" + trimmed[atIndex..];
        return trimmed;
    }

    private static PubspecLockParseResult Result(PubspecLockParseStatus status, FlutterProjectRootResult projectRoot,
        string? lockFilePath, string? rawText, string message)
