...
public sealed class PubspecLockParserValidationTests : IDisposable
{
    ...
    [InlineData("direct main", "[hosted]", "1.2.3")]
    [InlineData("direct main", "hosted", "[1.2.3]")]
    public void Parse_KnownPackageFieldHasWrongShape_ReturnsInvalidDocument(string dependencyValue, string sourceValue, string versionValue)
    {
        WriteLock(
            $"packages:\n" +
            $"  http:\n" +
    ...
    public void Parse_LockFileSymlinkOrReparsePoint_IsRejectedWhenPlatformSupportsIt()
    {
        ...
        try
        {
            try { File.CreateSymbolicLink(lockPath, outsideFile); }
            catch (Exception ex) when (ex is UnauthorizedAccessException or PlatformNotSupportedException or IOException) { return; }
            if ((File.GetAttributes(lockPath) & FileAttributes.ReparsePoint) == 0) return;
            var result = new PubspecLockParser().Parse(SuccessfulRoot());
    ...
    private void WriteLock(string content) => File.WriteAllText(Path.Combine(_root, "pubspec.lock"), content);
