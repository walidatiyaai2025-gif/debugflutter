...
public sealed class ProductFlavorDetectionTests : IDisposable
{
    private readonly string _root = Path.Combine(
        Path.GetTempPath(),
        "fbd-product-flavor-" + Guid.NewGuid().ToString("N"));

    public ProductFlavorDetectionTests() => Directory.CreateDirectory(_root);
    ...
    public void Detect_SingleDeclaredDimension_IsInferredWhenFlavorOmitsIt()
    {
        ...
        Assert.Equal("version", flavor.Dimension?.Value);
        Assert.Equal(ProductFlavorValueSourceKind.InferredSingleDimension, flavor.Dimension?.SourceKind);
        Assert.Contains(
            result.Evidence,
    ...
    [Fact]
    public void Detect_MultipleDimensionsAndMissingFlavorDimension_ReturnsPartial()
    {
        var result = Detect(
            """
            android {
              ...
              productFlavors {
                minApi24 { applicationIdSuffix ".api24" }
              }
            }
            """);
        ...
        Assert.Contains(ProductFlavorField.Dimension, flavor.UnresolvedFields);
    }

    [Fact]
    public void Detect_KotlinListOfDimensions_ParsesExplicitFlavorDimension()
    {
        var result = Detect(
            """
            android {
    ...
    [Fact]
    public void Detect_UnresolvedDimensionDeclaration_PreventsInferenceAndMarksFlavor()
    {
        var result = Detect(
            """
            android {
    ...
    [Fact]
    public void Detect_DynamicFlavorName_IsPartialWithoutGuessing()
    {
        var result = Detect(
            """
            android {
    ...
    [Fact]
    public void Detect_StaticPlusDynamicField_DoesNotSelectStaticAsEffective()
    {
        var result = Detect(
            """
            android {
    ...
    [Fact]
    public void Detect_CommentsStringsBuildTypesAndContainerHelpers_AreNotFlavors()
    {
        var result = Detect(
            """
    ...
    [Theory]
    [InlineData("android { defaultConfig { applicationId \"com.example.app\" } }")]
    [InlineData("android { productFlavors { } }")]
    [InlineData("android { productFlavors { all { } configureEach { } } }")]
    public void Detect_NoStaticFlavorDeclarations_ReturnsNoFlavors(string content)
    {
        var result = Detect(content);
