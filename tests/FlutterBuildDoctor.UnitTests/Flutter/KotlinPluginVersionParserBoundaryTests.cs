...
public sealed class KotlinPluginVersionParserBoundaryTests : IDisposable
{
    private readonly string _root = Path.Combine(Path.GetTempPath(), "fbd-kotlin-boundary-" + Guid.NewGuid().ToString("N"));

    public KotlinPluginVersionParserBoundaryTests() => Directory.CreateDirectory(_root);
    ...
    public void Parse_InjectedScriptPathOutsideExpectedRole_IsRejected()
    {
        ...
        File.WriteAllText(outside, "plugins { id 'org.jetbrains.kotlin.android' version '2.0.21' }");
        var dsl = Dsl(new GradleScriptEvidence(GradleScriptRole.ProjectBuild, GradleDslKind.Groovy, outside));

        var result = new KotlinPluginVersionParser().Parse(dsl);

        Assert.Equal(KotlinPluginVersionStatus.UnsafePath, result.Status);
    ...
    public void Parse_ScriptSymlink_IsRejectedWhenSupported()
    {
        var android = Path.Combine(_root, "android");
        Directory.CreateDirectory(android);
        var outside = Path.Combine(Path.GetTempPath(), "fbd-kotlin-outside-" + Guid.NewGuid().ToString("N") + ".gradle");
        File.WriteAllText(outside, "plugins { id 'org.jetbrains.kotlin.android' version '2.0.21' }");
        ...
        try
        {
            ...
            var result = new KotlinPluginVersionParser().Parse(
    ...
    [Fact]
    public void Parse_OversizeScript_IsRejectedBeforeVersionParsing()
    {
        var android = Path.Combine(_root, "android");
        Directory.CreateDirectory(android);
        ...
        Assert.Equal(KotlinPluginVersionStatus.FileTooLarge, result.Status);
        Assert.Null(result.Version);
    }
