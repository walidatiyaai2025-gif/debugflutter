...
public sealed class WindowsEnvironmentDetectorTests
{
    ...
    {
        var source = new StubSource(new WindowsRuntimeInfo(true, "Microsoft Windows 11 Pro", new Version(10, 0, 26100, 1234), Architecture.X64, Architecture.X64, true, true));
    ...
    {
        var source = new StubSource(new WindowsRuntimeInfo(true, "Microsoft Windows", new Version(10, 0, 26100), Architecture.Arm64, Architecture.X64, true, true));
        var result = new WindowsEnvironmentDetector(source).Detect();
        Assert.True(result.IsSuccess, result.Message);
    ...
    {
        var source = new StubSource(new WindowsRuntimeInfo(false, "Linux 6.8", new Version(6, 8), Architecture.X64, Architecture.X64, true, true));
        var result = new WindowsEnvironmentDetector(source).Detect();
    ...
    private sealed class StubSource : IWindowsRuntimeInfoSource
    {
        ...
        public StubSource(WindowsRuntimeInfo value) => _value = value;
