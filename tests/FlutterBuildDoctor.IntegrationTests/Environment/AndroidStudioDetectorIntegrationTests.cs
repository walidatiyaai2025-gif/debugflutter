...
public sealed class AndroidStudioDetectorIntegrationTests
{
    ...
    public void Detect_ComposesWithActualWindowsEvidenceAndBoundedLocalSearchRoot()
    {
        ...
        try
        {
            ...
            File.WriteAllText(
                Path.Combine(install, "product-info.json"),
                "{\"name\":\"Android Studio\",\"version\":\"2025.1.2\",\"buildNumber\":\"AI-251.12345\",\"productCode\":\"AI\"}");

            var windows = new WindowsEnvironmentDetector(new SystemWindowsRuntimeInfoSource()).Detect();
            var source = new WindowsAndroidStudioInstallationSource(new SingleRootProvider(
                new AndroidStudioSearchRoot(install, AndroidStudioDiscoverySource.LocalAppDataPrograms, Recursive: false)));
